...
class terminal {
    ...
    #create() {
        let terminal        = this.$ID(this.#element),
        ...
        `);
        this.$OBJ(terminal_title).ATTR('id', this.#hexID.title);
        this.$OBJ(terminal_title).TEXT(this.#attr.title);
        ...
        this.$OBJ(terminal_body).ATTR('style', `
            ...
            z-index: 1;
        `);
        this.$OBJ(terminal_body).ATTR('id', this.#hexID.body);
        ...
        this.$OBJ(terminal_text).ATTR('style', `
            ...
            float: left;
            max-width: 98%;
            max-height: calc(${this.#attr.height} - 45px);
            ...
            word-break: normal;
        `);
        this.$OBJ(terminal_text).ATTR('id', this.#hexID.text);
        ...
        this.$OBJ(terminal_input).ATTR('id', this.#hexID.input);
        ...
        this.$ID(this.#hexID.text).onfocus = (event) => {
            event.stopPropagation();
        }
        this.$ID(this.#hexID.text).onmousedown = (event) => {
            event.stopPropagation();
        }
        ...
        this.$ID(this.#hexID.input).onkeyup = (event) => {
        ...
        this.$ID(this.#hexID.input).onkeydown = (event) => {
            if(event.key == 'Enter')
            {
        ...
        this.$ID(this.#hexID.input).onmousedown = (event) => {
            event.preventDefault();
            event.stopPropagation();
    ...
    */
    ...
    #log() {
        ...
        if(arguments[0] != null) {
            ...
            if(this.#attr.dateOption == true)
            {
                ...
                if(this.#attr.typeMode == true || arguments[2] == true)
                {
                    for (let i = 0; i <= arguments[0].length + 13; i++) {
                        setTimeout(() => { this.$ID(logID).innerHTML += `${dateString} ${arguments[0].toString()}`.charAt(i); }, (i * 15));
                    }
                }
                else
                {
                ...
                if(this.#attr.typeMode == true || arguments[2] == true)
                {
                    for (let i = 0; i <= arguments[0].length; i++) {
                        setTimeout(() => { this.$ID(logID).innerHTML += arguments[0].charAt(i); }, (i * 15));
                    }
                }
                else
                {
            ...
            if(arguments[0].length > 0) { this.$ID(this.#hexID.text).innerHTML += `<div style="font-size:4px;"><br></div>`; }
     ...
     */
    addCommand() {
        if(!this.#attr.readOnly) this.#addCommand(arguments[0], arguments[1], arguments[2], arguments[3]);        
    }
    ...
    #defaultCommand() {       
        ...
        this.#addCommand('color', function () {
            ...
            if(typeof arguments[0][1] !== 'undefined') if(typeof arguments[0][1] !== 'string') { this.#log('arg type mismatch', '#CC0000'); }
            if(typeof arguments[0][2] !== 'undefined') if(typeof arguments[0][2] !== 'string') { this.#log('arg type mismatch', '#CC0000'); }

            if(this.#validateHex(arguments[0][1]))
            {
                this.$ID(this.#hexID.body).STYLE.backgroundColor = `rgba(${this.#hexRGB('#' + arguments[0][1])}, ${this.#attr.opacity})`;
            }
            if(this.#validateHex(arguments[0][2]))
            {
                this.$ID(this.#hexID.body).STYLE.color = `#${arguments[0][2]}`;
                this.$ID(this.#hexID.text).STYLE.color = `#${arguments[0][2]}`;
                this.$ID(this.#hexID.input).STYLE.color = `#${arguments[0][2]}`;
            }
        ...
        this.#addCommand('md2', function () {
            ...
            if(!this.$TYPE(arguments[0][1], 'empty'))
            {                
        ...
        this.#addCommand('systeminfo', async function () {            
            ...
            if(arguments[0].length > 1) { 
                if(typeof arguments[0][1] !== 'undefined') if(typeof arguments[0][1] === 'string') { 
        ...
        this.#addCommand('cookie', function () {
            ...
            if(arguments[0][1] == 'get') { this.#log(`${this.$COOKIE(arguments[0][2])}`); }
            else if(arguments[0][1] == 'set') {
                if(typeof arguments[0][3] !== 'undefined') if(typeof arguments[0][2] !== 'string') { this.$ERROR('arg type mismatch'); } else {
                    this.$COOKIE(arguments[0][2], arguments[0][3], 1);
                }                
            }               
    ...
    #lock() {        
        if(arguments[0])
        {
            this.$ID(this.#hexID.input).contentEditable = false;
            this.$ID(this.#hexID.input).STYLE.display = 'none';            
        }
        else
        {
            this.$ID(this.#hexID.input).contentEditable = true;
            this.$ID(this.#hexID.input).STYLE.display = 'inline-block';
            this.$ID(this.#hexID.input).focus();
        }
    }
